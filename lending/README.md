...
```bash
npm add ryan-lending
```

### PNPM

```bash
pnpm add ryan-lending
```

### Bun

```bash
bun add ryan-lending
```
...
- Command:
```sh
npx -y --package ryan-lending -- mcp start --api-key ... 
```
...
```sh
npx -y --package ryan-lending -- mcp start --help
```
...
import {
  ...
  SDKValidationError,
} from "ryan-lending/models/errors";
...
async function run() {
  let result;
  try {
    result = await lendingSDK.pet.petsStoreMonday({
      id: 10,
      name: "doggie",
      category: {
        id: 1,
...
```typescript
import { LendingSDK } from "ryan-lending";
import { HTTPClient } from "ryan-lending/lib/http";
