...
import {
  ...
  SDKValidationError,
} from "ryan-finance/models/errors";
...
async function run() {
  let result;
  try {
    result = await financeSDK.pet.petsStoreMonday({
      id: 10,
      name: "doggie",
      category: {
        id: 1,
...
```typescript
import { FinanceSDK } from "ryan-finance";
import { HTTPClient } from "ryan-finance/lib/http";
...
const sdk = new FinanceSDK({ httpClient });
```
